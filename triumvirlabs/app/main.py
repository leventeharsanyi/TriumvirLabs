...
def main() -> None:
    ...
    audio_file = st.audio_input("Record your question here:", sample_rate=48000, key="question_audio")
    if audio_file is not None:
        input_question = generate_text(audio_file)
        with st.chat_message("human"):
            st.markdown(f"**Transcript** 📝\n\n> {input_question}")
        if st.button("💬 Ask the agent", type="primary", use_container_width=True):
            with st.spinner("Thinking and searching EHR data to answer your question..."):
